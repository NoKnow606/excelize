...
### 场景 4.1：发起 workbook 全量重算，且 `A`、`B` 都有公式；同时读取 `A` 或 `B`

- 结果：读取 `A` 可能阻塞，读取 `B` 也可能阻塞
- 原因：`RecalculateAllWithDependency()` 会对整本 workbook 的公式做计算并写回
- 写回 `A` 时会持有 `A.ws.mu`
- 写回 `B` 时会持有 `B.ws.mu`
...
- 不是 `A.ws.mu` 直接把 `B.ws.mu` 锁住
- 而是 workbook 全量重算本身会依次或并行触达多张有公式的 sheet
- 所以从外部观察上，`A` 和 `B` 的读取都有机会被各自 sheet 的写回阻塞

更准确地说：

- 这不是“整个 workbook 一把总锁导致全部 worksheet 永久不可读”
- 而是“只要某张 sheet 在此次全量重算中会被写回，它的读取就可能与该 sheet 的写锁竞争”

另外仍需注意：

- 除了 worksheet 锁竞争，还可能有短暂的 `File.mu` 竞争
