...
// SetSQLSourceResolver configures source resolution for SQL formulas.
// Callers can install a resolver to support non-standard sheet tokens
// such as gid_* while keeping the SQL engine inside excelize.
func (f *File) SetSQLSourceResolver(resolver SQLSourceResolver) *File {
	f.sqlSourceResolver = resolver
	return f
}
